...
// ═══════════════════════════════════════════════════════════════
//  APP — Main component (see architecture comments in README)
// ═══════════════════════════════════════════════════════════════
...
export default function App() {
  ...
  // Capabilities
  useEffect(() => {
    if (!window.api) { setUiError('Backend bridge unavailable'); return }
    window.api.getCapabilities?.().then((r) => {
      if (r?.capabilities) { setCaps(r.capabilities); setInstallHints(r.install_hints || {}) }
    })
  }, [])
  ...
  useEffect(() => {
    const d = (e) => {
      if (e.code === 'Space') { e.preventDefault(); setHoldOriginal(true) }
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (r?.error) { setUiError(r.error); return }
 ...
 const handleInstall = useCallback(async (packageKey) => {
    if (!window.api?.installPackage) {
      pushToast('error', 'Missing', 'installPackage not in preload')
      return
    }
    setInstalling(packageKey)
    pushToast('ok', 'Installing', `Installing ${packageKey}… this may take a minute`)
    try {
      ...
      if (r?.error) {
        ...
        const c = await window.api.getCapabilities?.()
        if (c?.capabilities) {
      ...
      pushToast('error', 'Install error', e?.message || 'Unknown')
    } finally {
      setInstalling(null)
    }
  }, [pushToast])
  ...
  const handleAutoEnhance = useCallback(async () => {
    if (!originalPath) return
    setBusy(true); setBusyMsg('AI Auto-Enhancing')
    try {
      const r = await window.api.autoEnhance(originalPath)
      if (r?.error) { setUiError(r.error); return }
      if (r?.temp_path) {
        setTempPath(r.temp_path); setResultPreview(r.preview)
        const steps = (r.steps || []).map((s) => s.step).join(' → ')
        setResultLabel(`Auto: ${steps}`)
        if (r.metrics) setMetrics(r.metrics)
        pushToast('ok', 'Auto-enhanced', steps)
      }
    } finally { setBusy(false); setBusyMsg('') }
  }, [originalPath, pushToast])
  ...
  const handleUpscale = useCallback(async (scale = 4) => {
    ...
    setBusy(true); setBusyMsg(`Upscaling ${scale}×`)
    try {
      const r = await window.api.upscaleImage(src, scale)
      if (r?.error) { setUiError(r.error); return }
  ...
  }, [tempPath, originalPath, pushToast])

  const handleFaceRestore = useCallback(async () => {
    const src = tempPath || originalPath; if (!src) return
    setBusy(true); setBusyMsg('Restoring faces')
    try {
      const r = await window.api.faceRestore(src, 'gfpgan', 0.7)
      if (r?.error) { setUiError(r.error); return }
  ...
  const handleBgRemove = useCallback(async () => {
    ...
    try {
      const r = await window.api.bgRemove(src)
      if (r?.error) { setUiError(r.error); return }
      if (r?.temp_path) { setTempPath(r.temp_path); setResultPreview(r.preview); setResultLabel('BG Removed'); pushToast('ok', 'Background removed', '') }
    } finally { setBusy(false); setBusyMsg('') }
  }, [tempPath, originalPath, pushToast])
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      else { if (r?.error !== 'Cancelled') setUiError(r?.error); setSaving(false) }
    } catch (e) { setUiError(e?.message); setSaving(false) }
  }, [tempPath, pushToast])
  ...
  useEffect(() => {
    ...
    adjTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        if (r?.temp_path) { setTempPath(r.temp_path); setResultPreview(r.preview); setResultLabel('Live') }
  ...
  const handleApply = useCallback(async () => {
    if (!originalPath || !dirty) return; setBusy(true); setBusyMsg('Applying')
    try {
      const r = await window.api.adjustImage(originalPath, adj)
    ...
    <div style={S.app}>
      <header style={S.topBar}>
        <div style={S.topLeft}>
          ...
          <div>
            <div style={S.title}>Aurora Ops</div>
            <div style={S.sub}>{fileName || 'AI Image Engine'}</div>
          </div>
        </div>
        <div style={S.topRight}>
          ...
          <TopBtn onClick={handleSave} disabled={!tempPath || saving === true}>
            {saving === 'done' ? '✓' : saving ? '…' : 'Save'}
          </TopBtn>
      ...
      {uiError && <div style={S.errBar}><span style={{ fontWeight: 800 }}>ERROR</span> <span style={{ flex: 1, opacity: 0.9 }}>{uiError}</span> <button onClick={() => setUiError('')} style={S.errX}>✕</button></div>}
      ...
      <div style={S.body}>
        <aside style={S.sidebar}>
          ...
          <div style={S.panelArea}>
            ...
            {tab === 'adjust' && <div style={S.panelInner}>
            ...
            {tab === 'enhance' && <div style={S.panelInner}>
              <PH title="Enhance" sub="Stackable layers" />
              <FR l="Pipeline" v={pipeline || '—'} />
              <Sep />
             ...
             <AIBox
              ...
              <AIBox title="AI Upscale" desc="Real-ESRGAN 2×/4× neural detail synthesis"
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <GBtn onClick={() => handleUpscale(2)} disabled={!originalPath || busy || !caps.upscale_realesrgan}>2×</GBtn>
                  <ABtn onClick={() => handleUpscale(4)} disabled={!originalPath || busy || !caps.upscale_realesrgan}>4×</ABtn>
                </div>
              </AIBox>
              <AIBox title="Face Restore" desc="GFPGAN: fix blurry/damaged faces"
                ready={caps.face_gfpgan} hint={installHints.face_gfpgan}
                packageKey="gfpgan" onInstall={handleInstall} installing={installing}
                onClick={handleFaceRestore} dis={!originalPath || busy} />
              <AIBox title="BG Remove" desc="U2-Net: transparent PNG output"
                ready={caps.bg_remove} hint={installHints.bg_remove}
                packageKey="rembg" onInstall={handleInstall} installing={installing}
                onClick={handleBgRemove} dis={!originalPath || busy} />
              <AIBox title="Inpaint" desc="Paint mask to erase objects"
                ready={true} dis={true} hint="Draw-to-erase coming soon" />
              <Sep />
              <div style={S.hint}>AI tools process current result (or original). Chain: Adjust → Enhance → Upscale → Face.</div>
            </div>}
            {tab === 'analyze' && <div style={S.panelInner}>
              ...
              {rec?.ai_suggestions?.length > 0 && <><Sep /><SH>AI Suggestions</SH>{rec.ai_suggestions.map((s, i) => <div key={i} style={S.sugCard}><div style={S.sugTitle}>{s.action.replace('_', ' ')}</div><div style={S.sugText}>{s.reason}</div></div>)}</>}
              {rec?.best_mode && <><Sep /><div style={S.recCard}><FR l="Best Mode" v={rec.best_mode.toUpperCase()} /><div style={S.recWhy}>{rec.mode_reason}</div><GBtn onClick={() => { setActiveLayers([rec.best_mode]); setTab('enhance') }}>Apply → Enhance</GBtn></div></>}
            </div>}

            {tab === 'export' && <div style={S.panelInner}>
              <PH title="Export" sub="Save processed image" />
              <FR l="Result" v={resultLabel || (hasResult ? 'Ready' : '—')} />
              <Sep />
...
const Toggle = ({ label, on, flip }) => (
  <div style={S.tog}><span style={S.togL}>{label}</span><button onClick={flip} style={{ ...S.togBtn, ...(on ? S.togOn : {}) }}>{on ? 'ON' : 'OFF'}</button></div>
)

const ABtn = ({ children, onClick, disabled }) => <button onClick={onClick} disabled={disabled} style={{ ...S.aBtn, ...(disabled ? S.dis : {}) }}>{children}</button>
...
const StatusPill = ({ label, tone }) => {
  const c = { dim: '#94a3b8', cyan: '#67e8f9', amber: '#fcd34d', green: '#86efac' }[tone] || '#94a3b8'
  return <div style={{ ...S.sp, color: c, borderColor: c + '44', background: c + '18' }}>{label}</div>
}

const Pills = ({ items, value, onChange }) => (
  <div style={S.pills}>{items.map((i) => <button key={i.id} onClick={() => onChange(i.id)} style={{ ...S.pBtn, ...(value === i.id ? S.pOn : {}) }}>{i.label}</button>)}</div>
)

const Sl = ({ l, v, mn, mx, st = 1, f }) => {
  const ctr = mn < 0, pct = ((v - mn) / (mx - mn)) * 100
  const cp = ctr ? ((0 - mn) / (mx - mn)) * 100 : 0
  const left = ctr ? Math.min(pct, cp) : 0, width = ctr ? Math.abs(pct - cp) : pct
  return (
    <div style={S.slRow}>
      <div style={S.slHead}><span style={S.slL}>{l}</span><span style={S.slV}>{st < 1 ? (+v).toFixed(2) : Math.round(v)}</span></div>
      <div style={S.slTrack}>
        <div style={S.slBg} />
        <div style={{ ...S.slFill, left: `${left}%`, width: `${width}%` }} />
        {ctr && <div style={{ ...S.slMid, left: `${cp}%` }} />}
        <input type="range" min={mn} max={mx} step={st} value={v} onChange={(e) => f(parseFloat(e.target.value))} onDoubleClick={() => f(ctr ? 0 : mn)} style={S.slIn} />
      </div>
    </div>
  )
}
...
const Empty = ({ onOpen }) => (
  <div style={S.empty}><div style={S.emI}>✦</div><div style={S.emT}>Aurora Ops</div><div style={S.emS}>Load an image to begin</div><div style={{ height: 16 }} /><ABtn onClick={onOpen}>Open Image</ABtn></div>
)
...
const Toast = ({ t }) => {
  const c = t.type === 'ok' ? '#4ade80' : t.type === 'error' ? '#f87171' : '#60a5fa'
  return <div style={{ ...S.toast, borderColor: c + '44' }}><span style={{ fontWeight: 900, color: c }}>{t.title}</span> <span style={{ opacity: 0.9 }}>{t.message}</span></div>
}
...
bWarn: {
  ...
  panelInner: { height: '100%', overflowY: 'auto', padding: '12px 12px 20px' },

  ph: { marginBottom: 12 }, phT: { fontSize: 14, fontWeight: 950 }, phS: { fontSize: 11, color: '#64748b', marginTop: 2 },
  ...
  frL: { color: '#64748b', fontWeight: 800 }, frV: { fontWeight: 800, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  ...
  // Layer cards
  lCard: { width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 12, border: `1px solid ${L}`, background: 'rgba(2,6,23,0.2)', cursor: 'pointer', textAlign: 'left', marginBottom: 6 },
  lOn: { borderColor: 'rgba(59,130,246,0.30)', background: 'rgba(59,130,246,0.08)' },
  lIcon: { fontSize: 18, width: 28, textAlign: 'center' },
  lName: { fontSize: 12, fontWeight: 900 }, lDesc: { fontSize: 11, color: '#64748b', fontWeight: 700 },
  ...
  aiT: { fontSize: 12, fontWeight: 950 }, aiD: { fontSize: 11, color: '#64748b', marginTop: 2, lineHeight: 1.4 },
  aiH: { fontSize: 11, color: '#fbbf24', marginTop: 8, padding: '6px 8px', borderRadius: 8, background: 'rgba(251,191,36,0.08)', fontFamily: "'JetBrains Mono',monospace", wordBreak: 'break-all' },
  ...
  mcL: { fontSize: 10, color: '#64748b', fontWeight: 800, textTransform: 'uppercase' }, mcV: { fontSize: 14, fontWeight: 950, marginTop: 4 },
  ...
  sugTitle: { fontSize: 11, fontWeight: 900, color: '#93c5fd', textTransform: 'uppercase' }, sugText: { fontSize: 11, color: '#94a3b8', marginTop: 2 },
  recCard: { borderRadius: 12, border: `1px solid ${L}`, padding: 12 }, recWhy: { fontSize: 11, color: '#64748b', margin: '6px 0' },
  ...
  emT: { fontSize: 22, fontWeight: 950 }, emS: { fontSize: 13, color: '#64748b' },
...
// Keyframes
if (typeof document !== 'undefined' && !document.getElementById('__a_css')) {
  const el = document.createElement('style'); el.id = '__a_css'
  el.textContent = `@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}} *{box-sizing:border-box;margin:0} body{overflow:hidden}`
